 ...
 * 
 * GAS COSTS (FLAG_ONLY mode): ~60% lower write costs than FULL. No arrays stored.
 * GAS COSTS (COMPRESSED mode): ~50% lower write costs than FULL. Hash chain only.
 * 
 ...
 * 
 * HISTORY MODES (set at construction, immutable):
 *   FULL (0):       Complete arrays. All queries work. Highest gas. Default.
 *   FLAG_ONLY (1):  Only hasEverOwned() works. No arrays. Lowest gas. ~60% savings/transfer.
 *   COMPRESSED (2): Hash chain commitment. hasEverOwned() + off-chain verification. ~50% savings.
 * 
 * L1 vs L2 ECONOMICS:
 ...
 * 
 * ANTI-GRIEFING:
 * - Dual Sybil guards: intra-TX (EIP-1153 transient storage, namespaced keys) + inter-TX (same-block)
 * - Configurable transfer cooldown: setTransferCooldown(blocks) for L2 cross-block griefing
 * - Three history modes: FULL/FLAG_ONLY/COMPRESSED to control storage growth profile
 * - Pagination: bounded slices for all unbounded arrays
 * 
...
contract ERC721H is IERC721H, IERC721, IERC721Metadata { 
    ...
    /// @notice Minimum number of blocks between consecutive transfers of the same token.
    /// @dev 0 (default) = only same-block protection. Set higher on L2 to mitigate
    ///      cross-block churn griefing where an attacker transfers once per block for
    ///      thousands of blocks to inflate history storage.
    ///      Recommended values:
    ///        L1 (Mainnet):  0 (gas cost is natural deterrent)
    ...
    /// @notice Constructor with configurable history mode.
    /// @dev Mode is immutable after deployment.
    ///      FULL (0):       Complete arrays — all queries work. Highest gas.
    ///      FLAG_ONLY (1):  Only hasEverOwned(). No arrays. ~60% gas savings per transfer.
    ///      COMPRESSED (2): Hash chain commitment. On-chain proof, off-chain reconstruction.
    /// @param name_   Token name
    /// @param symbol_ Token symbol
    /// @param mode_   History mode: FULL(0), FLAG_ONLY(1), COMPRESSED(2)
    constructor(string memory name_, string memory symbol_, ERC721HStorageLib.HistoryMode mode_) {
        name = name_;
        symbol = symbol_;
        _nextTokenId = 1;
        owner = msg.sender;
    ...
    /// @dev Uses EIP-1153 transient storage (Cancun+). Each tokenId gets a NAMESPACED tstore slot.
    ...
    ///
    ///      KEY DESIGN: The transient storage key is keccak256(tokenId, ERC721H_TRANSFER_GUARD)
    ///      rather than raw tokenId, to prevent collision with other libraries or contracts
    ///      that use transient storage in the same transaction context (EIP-1153 is global).
    modifier oneTransferPerTokenPerTx(uint256 tokenId) {
        assembly {
            // Namespace the transient storage key to prevent cross-library collisions.
            // Salt: keccak256("ERC721H.oneTransferPerTokenPerTx") — a compile-time constant.
            // Slot = keccak256(abi.encode(tokenId, NAMESPACE_SALT))
            let ptr := mload(0x40)
            mstore(ptr, tokenId)
            mstore(add(ptr, 0x20), 0x7b4ca3dea54db2f0d920a45a336e363ae1655d6f52e8e62c5fb53cfe1ac19f83) // keccak256("ERC721H.oneTransferPerTokenPerTx")
    ...
    function _transfer(address from, address to, uint256 tokenId) internal nonReentrant oneTransferPerTokenPerTx(tokenId) {
        ...
        // SYBIL GUARD: Inter-TX protection — ensures minimum time between transfers.
        // (1) Same-block guard: derived from _ownershipBlocks — zero additional storage.
        if (_history.isSameBlockTransfer(tokenId, block.number)) {
            revert OwnerAlreadyRecordedForBlock();
        }
        // (2) Cooldown guard: enforces configurable minimum blocks between transfers.
        //     Mitigates L2 cross-block churn griefing (transfer once per block × N blocks).
        //     When transferCooldownBlocks == 0, this check is a no-op (default, L1-safe).
        {
            uint256 cooldown = transferCooldownBlocks;
            if (cooldown > 0) {
                uint256 lastBlock = _history.getLastTransferBlock(tokenId);
                if (lastBlock > 0 && block.number < lastBlock + cooldown) {
    ...
    /// @dev In COMPRESSED mode, each transfer updates: Hₙ = keccak256(Hₙ₋₁ ‖ owner ‖ block ‖ timestamp).
    ///      Returns bytes32(0) in FULL or FLAG_ONLY mode.
    function getHistoryHash(uint256 tokenId) public view returns (bytes32) {
        return _history.getHistoryHash(tokenId);
    }
    ...
    /// @notice Set minimum blocks between consecutive transfers of the same token.
    /// @dev Anti-griefing control for L2 deployments. Set to 0 to disable (same-block guard only).
    ///      Does NOT affect minting, only subsequent transfers.
    /// @param blocks Number of blocks to require between transfers (0 = default behavior)
    function setTransferCooldown(uint256 blocks) external onlyOwner {
